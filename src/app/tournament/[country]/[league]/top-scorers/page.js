...
export const generateMetadata = ({ params }) => {
    ...
    return {
        title: `Лучшие бомбардиры ${league[0]?.name.ru} ${country[0]?.name.ru} - рейтинг игроков`,
        description: `Список лучших бомбардиров ${league[0]?.name.ru} ${country[0]?.name.ru}. Узнайте, кто лидирует по количеству голов.`,
        keywords: `лучшие бомбардиры ${league[0]?.name.ru}, рейтинг игроков ${country[0]?.name.ru}, лидеры по голам`,
        openGraph: {
          type: 'website',
          title: `Лучшие бомбардиры ${league[0]?.name.ru} ${country[0]?.name.ru} - рейтинг игроков`,
          description: `Список лучших бомбардиров ${league[0]?.name.ru} ${country[0]?.name.ru}. Узнайте, кто лидирует по количеству голов.`
    ...
    return (
        <div className='league-top-scores tournament-country tournamentBlock'>
            <div className="head">
                <h2 className="left">
                    <div className="name">
                        <Image className='country-flag' src={country[0].flag} width={22} height={16} alt={'флаг ' + country[0].name.ru} title={country[0].name.ru} /> <Link href={`/tournament/${country[0].name.en.replaceAll(/\s+/g, '-').toLowerCase()}`} className='country-name'>{country[0].name.ru}</Link><span className='slash'>/</span><Image className='league-logo' src={league[0].logo} width={22} height={16} alt={'лого ' + league[0].name.ru} title={league[0].name.ru} /><Link href={`/tournament/${country[0].name.en.replaceAll(/\s+/g, '-').toLowerCase()}/${league[0].name.en.replaceAll('.', '').replaceAll('-', '').replaceAll(/\s+/g, '-').toLowerCase().replaceAll('ü', 'u').replaceAll('ə', 'a').replaceAll('ö', 'o').replaceAll('ğ', 'gh').replaceAll('ı', 'i').replaceAll('ç', 'c').replaceAll('ç', 'c').replaceAll('ã', 'a').replaceAll('ş', 's').replaceAll('ó', 'o').replaceAll('ú', 'u').replaceAll('é', 'e').replaceAll('ý', 'y').replaceAll('ï', 'i').replaceAll('á', 'a').replaceAll('ø', 'o').replaceAll('í', 'i').replaceAll('/', '-').replaceAll('\'', '')}`} className='country-name league-name'>{league[0].name.ru}</Link><span className='league-name'> <span className='slash'>/</span> Бомбардиры</span>
                    </div>
                </h2>
            </div>
            <TopScores placement={'top-scores-single'} leagueId={league[0].id} currentSeason={currentSeason && currentSeason.response[0].seasons[0].year} />
        </div>
    );
};
