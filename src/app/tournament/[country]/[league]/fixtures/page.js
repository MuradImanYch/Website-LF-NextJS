...
export const generateMetadata = ({ params }) => {
    ...
    return {
        title: `Расписание матчей ${league[0]?.name.ru} ${country[0]?.name.ru} - предстоящие игры`,
        description: `Актуальное расписание предстоящих матчей ${league[0]?.name.ru} ${country[0]?.name.ru}. Узнайте, когда будут проходить важные игры.`,
        keywords: `расписание матчей ${league[0]?.name.ru}, предстоящие игры ${country[0]?.name.ru}, календарь матчей`,
        openGraph: {
          type: 'website',
          title: `Расписание матчей ${league[0]?.name.ru} ${country[0]?.name.ru} - предстоящие игры`,
          description: `Актуальное расписание предстоящих матчей ${league[0]?.name.ru} ${country[0]?.name.ru}. Узнайте, когда будут проходить важные игры.`
    ...
    return (
        <div className='league-fixtures tournament-country tournamentBlock'>
            <div className="head">
                <h2 className="left">
                    <div className="name">
                        <Image className='country-flag' src={country[0].flag} width={22} height={16} alt={'флаг ' + country[0].name.ru} title={country[0].name.ru} /> <Link href={`/tournament/${country[0].name.en.replaceAll(/\s+/g, '-').toLowerCase()}`} className='country-name'>{country[0].name.ru}</Link><span className='slash'>/</span><Image className='league-logo' src={league[0].logo} width={22} height={16} alt={'лого ' + league[0].name.ru} title={league[0].name.ru} /><Link href={`/tournament/${country[0].name.en.replaceAll(/\s+/g, '-').toLowerCase()}/${league[0].name.en.replaceAll('.', '').replaceAll('-', '').replaceAll(/\s+/g, '-').toLowerCase().replaceAll('ü', 'u').replaceAll('ə', 'a').replaceAll('ö', 'o').replaceAll('ğ', 'gh').replaceAll('ı', 'i').replaceAll('ç', 'c').replaceAll('ç', 'c').replaceAll('ã', 'a').replaceAll('ş', 's').replaceAll('ó', 'o').replaceAll('ú', 'u').replaceAll('é', 'e').replaceAll('ý', 'y').replaceAll('ï', 'i').replaceAll('á', 'a').replaceAll('ø', 'o').replaceAll('í', 'i').replaceAll('/', '-').replaceAll('\'', '')}`} className='country-name league-name'>{league[0].name.ru}</Link><span className='league-name'> <span className='slash'>/</span> Расписание матчей</span>
                    </div>
                </h2>
            </div>
            <Fixtures placement={'fixtures-single'} leagueId={league[0].id} currentSeason={currentSeason && currentSeason.response[0].seasons[0].year} />
        </div>
    );
};
