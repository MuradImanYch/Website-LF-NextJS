...
import '@/app/tournament/[country]/[league]/news/page/[page]/style.css';
...
export const generateMetadata = ({ params }) => {
    ...
    return {
        title: `${country[0]?.name.en} ${league[0]?.name.en} - Latest Events and Analysis - Page ${params.page}`,
        description: `Read the latest news and events of the ${country[0]?.name.en} ${league[0]?.name.en}. Match reviews, interviews, and analysis. Page ${params.page}`,
        keywords: `${country[0]?.name.en} ${league[0]?.name.en} news, ${league[0]?.name.en} football news, match reviews ${country[0]?.name.en}, Page ${params.page}`,
        openGraph: {
          type: 'website',
          title: `${country[0]?.name.en} ${league[0]?.name.en} - Latest Events and Analysis - Page ${params.page}`,
          description: `Read the latest news and events of the ${country[0]?.name.en} ${league[0]?.name.en}. Match reviews, interviews, and analysis. Page ${params.page}`
        ...
        alternates: {
          canonical: params.page === '1' && `${config.domain}/en/tournament/${country[0].name.en.replace(/\s+/g, '-').toLowerCase()}/${league[0].name.en.replaceAll('.', '').replaceAll('-', '').replaceAll(/\s+/g, '-').toLowerCase().replaceAll('ü', 'u').replaceAll('ə', 'a').replaceAll('ö', 'o').replaceAll('ğ', 'gh').replaceAll('ı', 'i').replaceAll('ç', 'c').replaceAll('ç', 'c').replaceAll('ã', 'a').replaceAll('ş', 's').replaceAll('ó', 'o').replaceAll('ú', 'u').replaceAll('é', 'e').replaceAll('ý', 'y').replaceAll('ï', 'i').replaceAll('á', 'a').replaceAll('ø', 'o').replaceAll('í', 'i').replaceAll('/', '-').replaceAll('\'', '')}/news`
        }
    };
};
...
const page = async ({params}) => {
    ...
    return (
        <div className='league-news tournament-country'>
            ...
            <NewsPage lang={'en'} pagPathName={pagPathName} param={{tags: league[0].category && league[0].category.join(',')}} placement={'tournament-news'} params={params} />
        </div>
    )
}
